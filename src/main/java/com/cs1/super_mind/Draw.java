...
public abstract class Draw {
    ...
    public static TreeNode CurNode;//当前选中节点
    public static int number;//节点编号
    ...
    public static void update_len(TreeNode a) {
        ...
        if (a.isRoot()) {
            ...
            TreeNode.LBlockLen = (max(len, RecH)); // 更新左子树的块长度为累加的长度或者固定高度中的较大值
            len = 0; // 重置长度为0
            ...
            a.setBlockLen(max(len, RecH)); // 更新节点的块长度为累加的长度或者固定高度中的较大值
        }
    }
    ...
    // 删除节点及其子树在面板上的显示(还没删除list里的)
    public static void DelNode(TreeNode a, AnchorPane A) {
    ...
    public static void GetDp() {
        ...
        for (int i = 1; i <= tmplist.size(); i++) {
            for (int j = 0; j <= sum; j++) {
                ...
                if (dp[i - 1][j] == 1)
                    // dp[i-1][j] 不能选的情况，这里不需要设置select[i][j]
                    dp[i][j] = 1;
            }
        }
        // 遍历得到二分集的差的最小值
        ...
        for (int i = 1; i <= tmplist.size(); i++) {
            ...
            else TreeNode.getRchildren().add(tmplist.get(i - 1));
        }
        tmplist.clear();
    }
